...
def verificar_anos_em_csv(diretorio):
    ...
    def eh_data_valida(data_string):
        try:
            pd.to_datetime(data_string, format='%d-%m-%Y', errors='raise')
            return True
        except ValueError:
            return False
        ...
        if arquivo.endswith('.csv'):
            ...
            if len(df) > 1:
                ...
                if eh_data_valida(data_primeira_linha):
                    ano_primeira_linha = pd.to_datetime(data_primeira_linha, format='%d-%m-%Y').year
                else:
                ...
                if eh_data_valida(data_ultima_linha):
                    ano_ultima_linha = pd.to_datetime(data_ultima_linha, format='%d-%m-%Y').year
                else:
                    ano_ultima_linha = None

                # Verificar os anos das datas
                if ano_primeira_linha != 2012 or ano_ultima_linha != 2023:
