...
public final class RepositoryWalkerTask implements Runnable {
    ...
    // The report is the csv file generated by the program after all observations of
    // a given project are finished.
    ...
    // Threads represents the number of visitors working concurrently on a given
    // revision
    public final int threads;

    public final int steps;

    // Option to allow the walker to collect metrics about a single point in a given
    // repository
    ...
    public void run() {
        ...
        try {
            ...
            if (!hash.isEmpty()) {
                summaries = walker.traverse(hash, threads);
            } else {
            ...
            summaries.forEach(s -> {
                ...
                s.errors.forEach((k, v) -> errors.append(k)
                        .append("\n")
                        .append(v)
                        .append("\n-----------------------\n"));
            });
            ...
            reportAndErrorsWriter(reportFile, reportErrors, content, errors);
    ...
    private void reportAndErrorsWriter(Path reportFile, Path reportErrors, StringBuilder content, StringBuilder errors) {
