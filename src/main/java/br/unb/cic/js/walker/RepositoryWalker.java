...
import br.unb.cic.js.miner.metrics.Summary.SummaryBuilder;
...
public class RepositoryWalker {
        ...
        public List<Summary> traverse(Date initial, Date end, int steps, int threads) throws Exception {
                ...
                logger.info("{} -- finished, took {}ms in average to collect each commit group", project,
                                profiler.average());
        ...
        private Summary collect(ObjectId head, Date current, Map<Date, ObjectId> commits, int threads) {
                ...
                try (Git git = new Git(repository)) {
                        ...
                        metrics.add(Metric.builder().name("date (dd-mm-yyyy)")
                                        .value(Formatter.format.format(current))
                                        .build());
                        ...
                        for (Path p : files) {
                                executor.submit(() -> {
                                        try {
                                                ...
                                                logger.error("Error processing file: " + p + " on revision: " + commit,
                                                                ex);
                        ...
                        metrics.add(Metric.builder().name("arrow-function-declarations")
                                        .value(visitor.getTotalArrowDeclarations().get())
                                        .build());
                        metrics.add(Metric.builder().name("let-declarations")
                                        .value(visitor.getTotalLetDeclarations().get())
                                        .build());
                        ...
                        metrics.add(Metric.builder().name("yield-declarations")
                                        ...
                                        .build());
                        metrics.add(Metric.builder().name("import-statements")
                                        .value(visitor.getTotalImportStatements().get())
                                        .build());
                        metrics.add(Metric.builder().name("promise-declarations")
                                        .value(visitor.getTotalNewPromises().get()).build());
                        metrics.add(Metric.builder().name("promise-all-and-then")
                                        .value(visitor.getTotalPromiseAllAndThenIdiom().get())
                                        .build());
                        ...
                        metrics.add(Metric.builder().name("spread-arguments")
                                        .value(visitor.getTotalSpreadArguments().get())
                                        .build());
                        ...
                        metrics.add(Metric.builder().name("object-destructuring")
                                        ...
                                        .build());
                        metrics.add(Metric.builder().name("errors").value(errors.size())
                                        .build());
                        metrics.add(
                                        Metric.builder().name("statements").value(
                                                        visitor.getTotalStatements().get())
                                                        .build());
                        ...
                        boolean allMetricsFilled = areAllMetricsFilled(summary);
                        ...
                        logger.error("Failed to collect data for project: {} on revision: {}", project, commit, ex);
        ...
        public boolean areAllMetricsFilled(SummaryBuilder summary) {
                List<Metric> metrics = summary.build().metrics;
